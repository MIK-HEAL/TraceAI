...
如果你只是想快速试用，本地仓库里也可以直接安装：

```bash
go install ./cmd/traceai
```

如果仓库已经发布到 GitHub 并使用正式模块路径，就可以支持：
...
```bash
# 查看最常用的 MCP 工具
./bin/traceai --store sqlite --db trace.db top-tools

# 查看最常用的函数
./bin/traceai --store sqlite --db trace.db top-functions --limit 20

# 查看哪个 Agent 调用最多
./bin/traceai --store sqlite --db trace.db top-agents

# 查看版本
./bin/traceai version

# 发布时注入版本信息
go build -ldflags "-X main.version=v0.1.0 -X main.buildCommit=git-sha -X main.buildDate=2026-06-21T00:00:00Z" -o bin/traceai ./cmd/traceai

# 查看总体统计
./bin/traceai --store sqlite --db trace.db stats

# 输出基础报表
./bin/traceai --store sqlite --db trace.db report --limit 5 --catalog tools.txt --trend-days 7

# 启动 Dashboard（默认只监听本机）
./bin/traceai --store sqlite --db trace.db dashboard --addr 127.0.0.1:8080

# 对外暴露时加 token
./bin/traceai --store sqlite --db trace.db dashboard --addr 0.0.0.0:8080 --token your-secret

# 查看运行状态
./bin/traceai --store sqlite --db trace.db status

# 健康检查
./bin/traceai --store sqlite --db trace.db health

# 指标输出
./bin/traceai --store sqlite --db trace.db metrics --format json

# 显式写入演示数据
./bin/traceai --store sqlite --db trace.db seed-demo

# 导出 top tools 为 CSV
./bin/traceai --store sqlite --db trace.db export top-tools --format csv --output top-tools.csv

# 导出统计快照为 JSON
./bin/traceai --store sqlite --db trace.db export stats --format json --output stats.json

# 导出周报快照
./bin/traceai --store sqlite --db trace.db export weekly-stats --format csv --output weekly-stats.csv

# 导出月报快照
./bin/traceai --store sqlite --db trace.db export monthly-stats --format json --output monthly-stats.json
```
