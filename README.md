...
```bash
# 查看最常用的 MCP 工具
./toollens --store sqlite --db trace.db top-tools

# 查看最常用的函数
./toollens --store sqlite --db trace.db top-functions --limit 20

# 查看哪个 Agent 调用最多
./toollens --store sqlite --db trace.db top-agents

# 查看总体统计
./toollens --store sqlite --db trace.db stats
```
...
- [ ] Web Dashboard
- [ ] 更多 Adapter（Claude Code / Cursor / LangChain / A2A）
- [ ] 工具热力图 & AI 行为画像
- [ ] 月度报告自动生成
- [ ] 零调用功能检测
- [ ] 高失败率自动告警
